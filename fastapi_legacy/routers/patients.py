...
def check_medical_role(user: User = Depends(get_current_user)):
...
async def get_all_patients(
    ...
    limit: int = 100,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(check_medical_role)
):
    patient_service = PatientService(db)
    # Service doesn't have get_all_patients? Let's check or just implement here.
    # Service has 'patient_service.py', let's check it in a sec, but calling db directly is fine too.
    ...
    result = await db.execute(
        ...
        .options(
            ...
            selectinload(Patient.ai_analyses)
        )
...
async def get_patient(
    ...
    await log_audit(db, user_id=current_user.id, target_id=patient_id, target_table="patients", action="VIEW")
